...
if (!baseUrl) throw new Error('Usage: PREVIEW_URL=https://... npx tsx scripts/qa-v015-live.ts');
...
async function previewMetrics(page: Page, templateName: string) {
  return await page.evaluate((name) => {
    ...
    return {
      ...
      pageBackplate: rootStyle?.backgroundImage || rootStyle?.background || '',
      heroBackplate: heroStyle?.backgroundImage || heroStyle?.background || '',
      cardBackplate: cardStyle?.backgroundColor || cardStyle?.background || '',
...
async function main() {
  ...
  result.modalActualPreviewVisible = await page.getByText('套用後網站 Hero').first().isVisible().catch(() => false);
  result.modalMobilePreviewVisible = await page.getByText('手機實際套用').first().isVisible().catch(() => false);
  result.screenshots.push(await screenshot(page, 'template-quick-preview-modal.png'));
  ...
  result.ok = Object.values(result.forbiddenCounts).every((v: any) => Number(v) === 0)
    && result.templates.every((t: any) => t.desktop.notPlainColorBackplate && t.desktop.h1Visible && t.desktop.h1Opacity >= 0.9 && t.desktop.h1LessOrEqual4Lines && t.desktop.notOneOrTwoCharsPerLine && t.desktop.subtitleReadable && t.desktop.ctaVisible && t.desktop.artworkDoesNotCoverText && t.desktop.noHorizontalOverflow && t.mobile.notPlainColorBackplate && t.mobile.h1Visible && t.mobile.h1Opacity >= 0.9 && t.mobile.h1LessOrEqual4Lines && t.mobile.notOneOrTwoCharsPerLine && t.mobile.subtitleReadable && t.mobile.ctaVisible && t.mobile.artworkDoesNotCoverText && t.mobile.noHorizontalOverflow)
    && result.mobile390.noHorizontalOverflow && result.mobile320.noHorizontalOverflow && result.backEditVisible && result.modalBackVisible && result.modalActualPreviewVisible && result.modalMobilePreviewVisible
  ...
  await browser.close();
  if (!result.ok) throw new Error('v0.1.5 Live QA failed; see qa-artifacts/v0.1.5/live-qa-result.json');
