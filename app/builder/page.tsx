...
function Overview({ data, onStart, onReset }: { data: SiteData; onStart: () => void; onReset: () => void }) {
  ...
  return <div className="grid gap-5"><Card className="bg-gradient-to-br from-white to-teal-50"><SectionHeader eyebrow="Overview" title="網站總覽" description="從左側選單編輯資料，右側正式 Preview 會即時同步。" /><div className="grid gap-3 md:grid-cols-4">{[['模板', templateNames[data.template]], ['商品', `${productCount} 項`], ['圖片', `${data.media.length} 張`], ['模組', `${Object.values(data.modules).filter(Boolean).length}/9 開啟`]].map(([label, value]) => <div key={label} className="rounded-3xl bg-white p-4 shadow-sm ring-1 ring-slate-200"><p className="text-xs font-black text-slate-400">{label}</p><b className="mt-1 block text-slate-900">{value}</b></div>)}</div><div className="mt-5 flex flex-wrap gap-2"><Button onClick={onStart}>開始編輯基本資料</Button><Button variant="secondary" onClick={onReset}>重設預設資料</Button><Badge tone="green">localStorage 自動儲存</Badge></div></Card><Card><SectionHeader title="交付檢查" description="此版本不新增登入、付款、資料庫或自訂網域，只改善視覺產品化與模板品質。" /><div className="grid gap-3 md:grid-cols-2">{['siteData 單一資料來源', '三模板切換', 'JSON 匯入匯出', 'ZIP 靜態網站匯出', '離線 index.html', '390px RWD'].map(item => <div key={item} className="rounded-2xl bg-slate-50 p-4 font-bold text-slate-700 ring-1 ring-slate-200">✓ {item}</div>)}</div></Card></div>;
}
