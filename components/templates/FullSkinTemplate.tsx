...
export function FullSkinTemplate({ data, variant = 'fresh' }: { data: SiteData; variant?: 'fresh' | 'premium' | 'playful' }) {
  ...
  return <main className={`store-template skin-${skin.family}`} data-template-id={data.galleryTemplateId || data.visual?.selectedTemplateId || data.template} data-skin-id={skin.id} data-skin-family={skin.family} data-full-skin="true" data-template-variant={variant} style={cssVars}>
    <SkinStyle data={data} />
    <div className="wrap">
