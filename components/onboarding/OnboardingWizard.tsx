...
export function OnboardingWizard({ open, onClose }: { open: boolean; onClose: () => void }) {
  ...
  const setBasicField = (field: keyof typeof basic, value: string) => {
    ...
    setBasic(nextBasic);
    if (showBasicSummary) setBasicErrors(validateOnboardingBasicInfo(nextBasic).errors);
  };
  const validateStep3 = () => {
  ...
  return (
    <div className="fixed inset-0 z-[80] overflow-y-auto bg-slate-950/60 p-4 backdrop-blur-sm" data-testid="onboarding-wizard">
      <div className="mx-auto my-6 w-full max-w-4xl rounded-[2rem] bg-white p-5 shadow-2xl md:p-8">
        ...
        <div className="mt-7 flex flex-col gap-3 sm:flex-row sm:justify-between"><Button variant="secondary" onClick={() => step === 1 ? onClose() : setStep(step - 1)}>{step === 1 ? '取消' : '上一步'}</Button>{step < 5 ? <Button onClick={goNext}>下一步</Button> : <Button onClick={finish} data-testid="complete-onboarding">建立店名片並進入 Builder</Button>}</div>
      </div>
    </div>
  );
}
