...
import { generateMobileHeroLayoutCssVariables } from './mobileHeroLayout';
import { generateTemplateLayoutCssVariables, getTemplateLayoutCssVariables } from './templateLayoutMetrics';
      ...
      : skin.page.texture === 'bubbles'
        ...
        : skin.page.texture === 'charcoal'
          ...
          : skin.page.texture === 'botanical'
            ...
            : 'none';
  return `
.store-template,.export-site{${generateTemplateLayoutCssVariables()};${generateMobileHeroLayoutCssVariables()};font-family:inherit;min-height:100%;width:100%;overflow:hidden;background:${skin.page.background};color:${skin.page.textColor}}
...
.store-template .wrap,.export-site.wrap{width:100%;max-width:var(--site-page-max,1440px);margin:0 auto;padding:0;display:grid;gap:28px}
.skin-nav{width:calc(100% - (var(--site-padding-x) * 2));max-width:var(--site-content-max,1320px);margin:24px auto 0;display:flex;align-items:center;justify-content:space-between;gap:18px;padding:18px 24px;background:${skin.nav.background};color:${skin.nav.textColor};border:${skin.nav.border};border-radius:${skin.nav.radius};box-shadow:${skin.nav.shadow};backdrop-filter:blur(16px)}.skin-nav b{font-size:clamp(20px,3vw,30px)}.skin-nav nav{display:flex;gap:14px;flex-wrap:wrap}.skin-nav span{color:${skin.nav.textColor};font-size:13px;font-weight:900;letter-spacing:.08em}.skin-nav small{color:${skin.nav.accentColor};font-weight:900}
.template-hero{width:100%;padding:0 var(--site-hero-padding-x);position:relative;isolation:isolate}.template-hero-inner{position:relative;isolation:isolate;overflow:hidden;width:100%;max-width:var(--site-hero-max,1360px);min-height:var(--artwork-hero-desktop,620px);margin:0 auto;border-radius:34px;padding:clamp(34px,5vw,72px);display:flex;align-items:center;border:${skin.hero.panelBorder};box-shadow:${skin.productCard.shadow};background:#111}.template-hero-backplate{position:absolute;inset:0;width:100%;height:100%;z-index:0;object-fit:var(--artwork-fit,cover);object-position:var(--artwork-position,${skin.artwork.objectPosition})}.mobile-hero-artwork-stage{display:none}.mobile-artwork-safe-frame{width:100%;max-width:100%;overflow:hidden;position:relative;box-sizing:border-box;border-radius:var(--mobile-artwork-radius,22px);background:rgba(255,255,255,.18)}.mobile-artwork-safe-frame img,.mobile-artwork-image{display:block;max-width:100%;width:100%;height:auto;box-sizing:border-box;margin:0;transform:none}.mobile-artwork-overlay{position:absolute;inset:0;pointer-events:none;background:var(--mobile-artwork-overlay,transparent);opacity:.18}.mobile-artwork-safe-frame[data-mobile-artwork-mode='contain-poster'] img{object-fit:contain;object-position:center center}.mobile-artwork-safe-frame[data-mobile-artwork-mode='top-contain'] img{object-fit:contain;object-position:top center}.mobile-artwork-safe-frame[data-mobile-artwork-mode='center-contain'] img{object-fit:contain;object-position:center center}.mobile-artwork-safe-frame[data-mobile-artwork-mode='safe-cover']{aspect-ratio:16/11}.mobile-artwork-safe-frame[data-mobile-artwork-mode='safe-cover'] img{height:100%;object-fit:cover;object-position:center center}.mobile-artwork-safe-frame[data-mobile-artwork-mode='cropped-window']{aspect-ratio:16/12}.mobile-artwork-safe-frame[data-mobile-artwork-mode='cropped-window'] img{height:100%;object-fit:cover;object-position:center center}.mobile-artwork-safe-frame[data-mobile-artwork-mode='background-soft'] img{object-fit:contain;opacity:.68;filter:saturate(.95) blur(.2px)}.template-hero-overlay{position:absolute;inset:0;z-index:1;background:var(--artwork-overlay,${skin.artwork.overlay})}.hero-copy{position:relative;z-index:2;width:min(54%,760px);min-width:560px;border-radius:28px;background:${skin.hero.panelBackground};padding:clamp(28px,4vw,48px);border:${skin.hero.panelBorder};box-shadow:0 24px 70px rgba(0,0,0,.22);backdrop-filter:blur(18px);overflow:visible}.template-hero[data-panel-position='bottom'] .template-hero-inner{display:block;padding:0;background:${skin.section.background}}.template-hero[data-panel-position='bottom'] .template-hero-backplate{position:relative;display:block;inset:auto;height:var(--artwork-hero-desktop,620px)}.template-hero[data-panel-position='bottom'] .template-hero-overlay{display:none}.template-hero[data-panel-position='bottom'] .hero-copy{width:min(calc(100% - 48px),980px);min-width:0;margin:18px auto 24px;padding:clamp(22px,3vw,36px)}.template-hero[data-panel-position='bottom'] .hero-copy h1{font-size:clamp(38px,4.1vw,62px)}.hero-copy h1{margin:0;max-width:100%;font-size:clamp(48px,5.4vw,82px);line-height:1.02;letter-spacing:-.045em;color:${skin.hero.headingColor};text-wrap:balance;white-space:normal;word-break:normal;overflow-wrap:normal}.hero-title-line{display:inline}.hero-copy .muted{color:${skin.hero.subtitleColor};max-width:100%;overflow-wrap:normal}.skin-link-list{max-width:100%}
.skin-eyebrow,.placeholder-badge{display:inline-flex;margin:0 0 12px;color:${skin.section.eyebrowColor};font-size:12px;font-weight:1000;letter-spacing:.24em;text-transform:uppercase}.section-title,.skin-section h2{margin:0 0 22px;color:${skin.section.headingColor};font-size:clamp(32px,3.6vw,54px);letter-spacing:-.03em}.muted{color:${skin.page.mutedTextColor};line-height:1.8}.skin-section{position:relative;overflow:hidden;width:calc(100% - (var(--site-section-padding-x) * 2));max-width:var(--site-section-max,1320px);margin:0 auto;border-radius:30px;background:${skin.section.background};padding:34px;border:${skin.section.border};box-shadow:${skin.productCard.shadow}}.skin-section.is-alt{background:${skin.section.alternateBackground}}.skin-section-inner{width:100%;max-width:none;margin:0 auto}.skin-section:after{content:"";display:block;height:1px;margin-top:24px;background:${skin.section.divider};opacity:.7}
.product-grid{display:grid;grid-template-columns:repeat(var(--site-grid-columns,4),minmax(0,1fr));gap:var(--site-grid-gap,24px);width:100%}.skin-product-card{background:${skin.productCard.background};color:${skin.productCard.textColor};border:${skin.productCard.border};border-radius:${skin.productCard.radius};box-shadow:${skin.productCard.shadow};padding:20px;overflow:hidden}.skin-product-card h3{margin:16px 0 8px;color:${skin.productCard.textColor};font-size:22px}.price{display:block;color:${skin.productCard.priceColor};font-size:24px;font-weight:1000}.product-img{display:block;width:100%;height:220px;object-fit:cover;border-radius:calc(${skin.productCard.radius} - 8px);border:0}
.themed-placeholder{position:relative;overflow:hidden;background:${skin.placeholder.background};min-height:220px}.themed-placeholder .placeholder-orb,.themed-placeholder .placeholder-line{position:absolute;display:block;background:${skin.placeholder.accent};opacity:.72}.themed-placeholder .orb-a{width:42%;aspect-ratio:1;border-radius:999px;right:10%;top:12%;filter:blur(1px)}.themed-placeholder .orb-b{width:28%;aspect-ratio:1;border-radius:999px;left:12%;bottom:13%;opacity:.38}.themed-placeholder .line-a{height:2px;width:62%;left:18%;top:46%;transform:rotate(-10deg)}.themed-placeholder .line-b{height:2px;width:46%;right:14%;top:60%;transform:rotate(12deg);opacity:.45}.themed-placeholder b{position:absolute;left:18px;bottom:14px;color:${skin.placeholder.textColor}}
.skin-menu-list{display:grid;grid-template-columns:repeat(2,minmax(0,1fr));gap:20px;width:100%;background:${skin.menuList.background};border-radius:26px;padding:22px}.skin-menu-category{padding:20px;border-radius:22px;background:${skin.menuList.background};border:${skin.menuList.rowBorder}}.skin-menu-category h3{margin:0 0 8px;color:${skin.menuList.categoryColor};font-size:24px}.skin-menu-row{display:flex;justify-content:space-between;gap:16px;align-items:flex-start;margin-top:10px;padding:14px 16px;border-radius:16px;background:${skin.menuList.rowBackground};border:${skin.menuList.rowBorder}}.skin-menu-row b{color:${skin.menuList.itemColor}}.skin-menu-row small{display:block;color:${skin.menuList.descriptionColor};margin-top:4px}.skin-menu-row strong{color:${skin.menuList.priceColor};font-size:20px}.info-grid{display:grid;grid-template-columns:repeat(2,minmax(0,1fr));gap:16px}.faq-list{display:grid;grid-template-columns:repeat(2,minmax(0,1fr));gap:16px}.skin-link-list{display:flex;flex-wrap:wrap;gap:10px}.skin-btn{display:inline-flex;min-height:44px;align-items:center;justify-content:center;border-radius:999px;padding:10px 18px;font-weight:1000;color:${skin.hero.ctaColor};background:${skin.hero.ctaBackground};box-shadow:0 12px 28px rgba(0,0,0,.18)}
.brand-story{min-height:180px;display:flex;align-items:center}.skin-footer{width:calc(100% - (var(--site-section-padding-x) * 2));max-width:var(--site-section-max,1320px);margin:0 auto 28px;border-radius:28px;background:${skin.footer.background};color:${skin.footer.textColor};padding:30px 34px;display:flex;justify-content:space-between;gap:16px;flex-wrap:wrap}.skin-footer b{color:${skin.footer.accentColor}}
@media(max-width:1180px){.store-template,.export-site{--site-grid-columns:3}.skin-menu-list{grid-template-columns:1fr}.hero-copy{min-width:500px;width:min(62%,720px)}}
@media(max-width:760px){.store-template,.export-site{${generateMobileHeroLayoutCssVariables()};overflow-x:hidden}.store-template .wrap,.export-site.wrap{padding:14px;gap:16px;max-width:100%;overflow-x:hidden}.skin-nav{width:100%;margin:0;border-radius:22px;min-width:0}.skin-nav nav{display:none}.template-hero{padding:0;overflow:hidden}.template-hero-inner{width:100%;max-width:100%;min-height:auto;padding:var(--mobile-hero-padding-x);align-items:center;justify-content:flex-start;flex-direction:column;gap:14px;border-radius:24px;overflow:hidden;background:${skin.section.background}}.template-hero-backplate,.template-hero-overlay{display:none}.mobile-hero-artwork-stage{display:block;width:100%;max-width:100%;overflow:hidden}.mobile-artwork-safe-frame{width:100%;max-width:100%;overflow:hidden;display:grid;place-items:center}.mobile-artwork-safe-frame img{display:block;max-width:100%;width:100%;height:auto;max-height:none;transform:none;margin:0;min-width:0}.hero-copy,.mobile-hero-content-panel{width:min(100%,340px);max-width:var(--mobile-hero-panel-max-width);min-width:0;margin:0 auto;box-sizing:border-box;padding:clamp(22px,6vw,32px);border-radius:28px;overflow:hidden}.hero-copy h1,.mobile-hero-title{width:100%;max-width:var(--mobile-hero-title-max-width);font-size:var(--mobile-hero-title-font-size);line-height:var(--mobile-hero-title-line-height);letter-spacing:-.02em;white-space:normal;word-break:normal;overflow-wrap:normal;line-break:strict;text-wrap:balance;display:-webkit-box;-webkit-line-clamp:var(--mobile-hero-title-max-lines);-webkit-box-orient:vertical;overflow:visible}.hero-title-line{display:block}.hero-copy .muted{font-size:var(--mobile-hero-subtitle-font-size);line-height:var(--mobile-hero-subtitle-line-height);white-space:normal;word-break:normal;overflow-wrap:normal}.skin-link-list{display:flex;flex-wrap:wrap;max-width:100%;gap:10px}.skin-btn{max-width:100%;white-space:normal;text-align:center;overflow-wrap:normal}.skin-section,.skin-footer{width:100%;padding:20px;border-radius:22px}.product-grid,.info-grid,.faq-list{grid-template-columns:1fr}.skin-menu-row{padding:12px;flex-direction:column}.product-img,.themed-placeholder{height:150px;min-height:150px}.skin-footer{margin-bottom:0}}@media(max-width:360px){.store-template,.export-site{${generateMobileHeroLayoutCssVariables(320)}}.hero-copy,.mobile-hero-content-panel{width:min(100%,272px);max-width:calc(100% - 40px);min-width:0;padding:18px}.template-hero-inner{padding:12px}}
`;
}
