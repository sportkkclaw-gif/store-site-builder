...
function renderHeroHtml(data: SiteData, skin: TemplateSkin, artwork: ReturnType<typeof getTemplateArtwork>, artworkFit: string, artworkPosition: string) {
  const titleLines = renderHeroTitleLinesHtml(data.hero.title, data.store.tagline);
  return `<section class="template-hero" data-testid="site-hero" data-skin-component="hero" data-skin-family="${esc(skin.family)}" data-visual-contract-id="${esc(skin.visualContractId)}" data-artwork-source="true" data-artwork-src="${esc(artwork.gallerySrc)}" data-panel="${esc(artwork.backplate.textPanelMode)}" data-contrast="${esc(artwork.backplate.textContrast)}" style="--artwork-overlay:${esc(artwork.backplate.overlay)};--artwork-hero-desktop:${esc(artwork.backplate.heroHeightDesktop)};--artwork-hero-mobile:${esc(artwork.backplate.heroHeightMobile)};--artwork-fit:${esc(artworkFit)};--artwork-position:${esc(artworkPosition)}"><div class="template-hero-inner" data-testid="site-hero-inner"><img class="template-hero-backplate" src="${esc(artwork.exportAssetPath || artwork.gallerySrc)}" data-artwork-src="${esc(artwork.gallerySrc)}" data-gallery-src="${esc(artwork.gallerySrc)}" alt=""><div class="template-hero-overlay"></div><div class="hero-copy mobile-hero-content-panel" data-testid="hero-content-panel"><span class="placeholder-badge">${esc(data.store.tagline)}</span><h1 data-testid="hero-title" aria-label="${esc(data.hero.title)}">${titleLines}</h1><p class="muted" data-testid="hero-subtitle">${esc(data.hero.subtitle)}</p><div class="skin-link-list" data-testid="hero-cta-row">${links(data)}</div></div></div></section>`;
}

function sectionHtml(className: string, skin: TemplateSkin, body: string) {
  return `<section class="${className}" data-testid="site-section" data-skin-component="section" data-skin-family="${esc(skin.family)}"><div class="skin-section-inner" data-testid="site-section-inner">${body}</div></section>`;
}
